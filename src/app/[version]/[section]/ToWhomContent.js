...
// export default function ToWhomContent() {
...
export default function ToWhomContent({ config, version }) {
  if (!config || !config.sections) return <p>No content found.</p>;

  const sectionKeys = Object.keys(config.sections);
  const sectionsWithRefs = sectionKeys.reduce((acc, key) => {
    acc[key] = {
      ...config.sections[key],
      ref: useRef(null),
    };
  ...
  return (
    <>
      <BannerSection
        ...
        buttonText="Schedule a Demo"
        buttonAction={() => alert(`Demo scheduled for ${version}`)}
      />
